...
export function VariantsManager({
  ...
  const [useGlobalPrices, setUseGlobalPrices] = useState(
    initialUseGlobalPrices ?? false
  );
  const [globalUnitPrice, setGlobalUnitPrice] = useState(
    initialGlobalUnitPrice ?? ""
  );
  const [globalPromotionalPrice, setGlobalPromotionalPrice] = useState(
    initialGlobalPromotionalPrice ?? ""
  );
  ...
  const handleAddVariant = () => {
    ...
    setCurrentVariant({
      ...defaultVariant,
      unit_price:
        useGlobalPrices && !isNaN(numericGlobalUnitPrice)
          ? numericGlobalUnitPrice
          : 0,
      promotionalPrice:
        useGlobalPrices && !isNaN(numericGlobalPromoPrice)
          ? numericGlobalPromoPrice
          : null,
      cost:
        useGlobalPrices && !isNaN(numericGlobalCost) ? numericGlobalCost : 0,
  ...
  const handleEditVariant = (variant: ProductVariant, index: number) => {
    ...
    setCurrentVariant({
      ...
      cost:
        useGlobalPrices && !isNaN(numericGlobalCost)
          ? numericGlobalCost
          : variant.cost,
    });
    setEditIndex(index);
  ...
  const handleSaveVariant = () => {
    ...
    if (
      !isVariantConfirmed(variantToSave) &&
      variantToSave.id &&
  ...
  const generateVariants = () => {
    ...
    const validProperties = properties.filter(
      (prop) =>
        ...
        prop.values.every((val) => val.value.trim() !== "")
    );
    ...
    console.log(
      "Combinaciones que se generarán:",
    ...
    const newVariants: ProductVariant[] = combinations.map((combination) => {
      ...
      const existingVariant = variants.find((variant) => {
        return Object.entries(combination).every(
          ([key, value]) => variant.attr[key] === value
        );
      });
      ...
      <Card className="mb-4">
        <CardHeader className="py-3">
          <CardTitle className="text-base flex items-center gap-3">
            ...
            />
            <Label htmlFor="use-global-prices" className="cursor-pointer">
              Usar precios globales para todas las variantes
            </Label>
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent className="max-w-[300px] text-xs">
                  <p>
                    Si activas esta opción, los precios definidos aquí se
                    aplicarán a todas las variantes nuevas y pre-completarán los
                    campos al editar. Los campos de precio individuales se
                    desactivarán.
                  </p>
                </TooltipContent>
              </Tooltip>
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-0">
          <div>
            <Label htmlFor="global-unit-price" className="text-sm mb-1 block">
          ...
          <div>
            <Label htmlFor="global-promo-price" className="text-sm mb-1 block">
              Precio promocional global
            </Label>
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground text-sm">
                $
          ...
          <div>
            <Label htmlFor="global-cost" className="text-sm mb-1 block">
              Costo global
            </Label>
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground text-sm">
            ...
            <div className="space-y-4">
              {properties.map((property) => (
                <div key={property.id} className="flex items-center gap-3">
                  <span className="font-medium min-w-[100px] text-sm capitalize">
                    {property.name}:
                  </span>
      ...
      {variants.length === 0 ? (
        <div className="text-center py-8 border rounded-md">
          <p className="text-muted-foreground">
            No hay variantes configuradas. Agrega propiedades y genera variantes
            para definir precios y stock.
          </p>
        </div>
      ) : (
                ...
                >
                    ...
                    {variant.imageId ? (
                      <div className="relative h-20 w-20">
                        <Image
                          src={
                            images.find(
                              (img) => img.id.toString() === variant.imageId
                            )?.src?.replace("original", "small") || "/placeholder.svg"
                          }
                          alt={variant.id}
                          fill
                  ...
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      {Object.entries(variant.attr).map(
                        ([key, value], index) => (
                          <Badge
                            key={`${variant.id}-${key}`}
                            variant="outline"
                  ...
                  <TableCell>
                    <Badge
                      variant={
                        isVariantConfirmed(variant) ? "default" : "secondary"
                      }
                    >
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button
                              ...
                              size="icon"
                              onClick={() =>
                                handleDeleteVariant(variants.indexOf(variant))
                              }
          ...
          >
            <div className="grid gap-6 py-4">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        ...
                        <TooltipContent className="max-w-[300px] text-xs">
                          <p>
                            Este es el IDC (Identificador Único del sistema
                            Contable). Es el ID seleccionado a esta variante en
                            particular.
                          </p>
                        </TooltipContent>
                      </Tooltip>
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        ...
                        <TooltipContent className="max-w-[200px] text-xs">
                          <p>
                            Esta funcionalidad es para asociar una imagen a una
                            variante.
                          </p>
                        </TooltipContent>
                      </Tooltip>
                    ...
                    <PopoverTrigger asChild>
                      <div
                        className={cn(
                          ...
                          !currentVariant || !isVariantConfirmed(currentVariant)
                            ? "opacity-50"
                            : "",
                          ...
                          <Image
                            src={
                              images
                                .find(
                                  (img) =>
                                    img.id.toString() === currentVariant.imageId
                                )
                                ?.src?.replace("original", "small") ||
                              "/placeholder.svg"
                            }
                        ...
                        {images.map((image) => (
                          <div
                            ...
                            className={cn(
                              ...
                              currentVariant?.imageId === image.id.toString()
                                ? "border-primary"
                                : "border-transparent hover:border-primary/50"
                            )}
                            onClick={() =>
                              updateVariantField("imageId", image.id.toString())
                            }
                            role="button"
                            tabIndex={0}
                          >
                            <Image
                              src={image.src?.replace("original", "small")}
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      <div key={property.id}>
                        <Select
                          ...
                          onValueChange={(value) =>
                            updateVariantField(`attr.${property.name}`, value)
                          }
                        >
                          <SelectTrigger
                          ...
                          >
                            <SelectValue
                              placeholder={`Seleccionar ${property.name.toLowerCase()}`}
                            />
                          </SelectTrigger>
                          <SelectContent>
                            {property.values.map(
              ...
              {stockManagement && (
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-2">
                    Cantidad de unidades disponibles para esta variante
                    específica.
                  </p>
                </div>
              )}
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-2">
                  Es de uso interno, tus clientes no lo verán
                </p>
              </div>
            </div>
